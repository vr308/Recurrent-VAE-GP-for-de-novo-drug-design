...
class RNN_encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # You can use the final hidden state or the entire sequence as the encoding
        # For example, using the final hidden state:
        encoding = hidden[-1, :, :]  # Take the last layer's hidden state
        
        # Map hidden state to mean and log variance
        mean = self.linear_mean(encoding)
        logvar = self.linear_logvar(encoding)
...
class RNN_decoder(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, latent_dimension, gru_stack_size, gru_neurons_num,
                 out_dimension):
        """
        Through Decoder
        """
    ...
    def forward(self, z, hidden):
        ...
        l1, hidden = self.decode_RNN(embed, hidden)
...
def train_model(rnn_encoder, rnn_decoder,
    ...
    optimizer_encoder = torch.optim.Adam(rnn_encoder.parameters(), lr=lr_enc)
    optimizer_decoder = torch.optim.Adam(rnn_decoder.parameters(), lr=lr_dec)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_iteration in range(num_batches_train):  # batch iterator
            ...
            #inp_flat_one_hot = batch.flatten(start_dim=1)
            latent_points, mus, log_vars = rnn_encoder(batch)
            ...
            hidden = rnn_decoder.init_hidden(batch_size=batch_size)
            
            # decoding from RNN N times, where N is the length of the largest
            # molecule (all molecules are padded)
            output_indices = torch.zeros_like(batch, dtype=torch.long)
            for seq_index in range(batch.shape[1]):
                output, hidden = rnn_decoder(latent_points, hidden)
                __, predicted_indices = torch.max(output, 2)
                output_indices[:, seq_index] = predicted_indices[:,0]
            

            # compute ELBO
            loss = compute_elbo(batch, output, mus, log_vars, KLD_alpha)

            # perform back propogation
            optimizer_encoder.zero_grad()
            optimizer_decoder.zero_grad()
            loss.backward(retain_graph=True)
            nn.utils.clip_grad_norm_(rnn_decoder.parameters(), 0.5)
            ...
            if batch_iteration % 30 == 0:
                ...
                # assess reconstruction quality
                quality_train = compute_recon_quality(batch, output_indices)
                quality_valid = quality_in_valid_set(rnn_encoder, rnn_decoder,
        ...
        if quality_increase == 1 and quality_valid_list[-1] > 50.:
            corr, unique = latent_space_quality(rnn_encoder, rnn_decoder,
...
def reconstuct(input_mols, vae_encoder, vae_decoder, largest_molecule_len, encoding_alphabet, type_of_encoding):
